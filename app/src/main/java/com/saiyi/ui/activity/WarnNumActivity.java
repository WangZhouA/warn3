...
public class WarnNumActivity extends BasActivity implements View.OnClickListener, HttpRequestCallback, WarnNmAdapterCallback {
...
//    WarnNumActivityAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        forPlayDialog = new ForPlayDialog(WarnNumActivity.this);
        forPlayDialog.setTitle(getResources().getString(R.string.to_contact_number));
        forPlayDialogToReName = new ForPlayDialog(WarnNumActivity.this);
        forPlayDialogToReName.setTitle(getResources().getString(R.string.toRE_contact_number));
        if (MyDevicesActivityAdapter.getIntentDerviceId() != null) {
            //     Toast.makeText(WarnNumActivity.this, MyDevicesActivityAdapter.getIntentDerviceId(), Toast.LENGTH_SHORT).show();
            intentDerviceId = MyDevicesActivityAdapter.getIntentDerviceId();
        }
        okHttpQureNumber();
        warnNumAdapter = new WarnNumAdapter(this, this);
...
//                Toast.makeText(WarnNumActivity.this, sequest, Toast.LENGTH_LONG).show();
                //添加到数据库
                break;
...
//                Toast.makeText(WarnNumActivity.this, sequest, Toast.LENGTH_LONG).show();
            ...
            case ADDPHONE_HTTP://添加给数据库_____添加
              if (sequest.contains("1")) {
                  Toast.makeText(WarnNumActivity.this, getResources().getString(R.string.ok), Toast.LENGTH_LONG).show();
              }else {
                  Toast.makeText(WarnNumActivity.this, sequest, Toast.LENGTH_LONG).show();
              }
                okHttpQureNumber();
    ...
    public void onFailure(String exp) {
        Toast.makeText(WarnNumActivity.this, getResources().getString(R.string.no), Toast.LENGTH_LONG).show();
    }
...
//                    addPhone(phone_one, position);
                    bigModle.getPutMac(WarnNumActivity.this, ConstantInterface.MAC, ConstantInterface.MAC + "10" + "0" + (position ) + phone_one + ",1", ADDPHONE_MAC, WarnNumActivity.this);
                }
            });
...
//                    editPhone(phone_ids, phone_one, position);
                    bigModle.getPutMac(WarnNumActivity.this, ConstantInterface.MAC, ConstantInterface.MAC + "10" + "0" + (position ) + phone_one + ",1", EDITPHONE_MAC, WarnNumActivity.this);
                }
            });
