...
public class PercentageRing extends View{
	...
	public PercentageRing(Context context, AttributeSet attrs) {
		...
		//中间圆的半径 默认为60
		mRadius = typedArray.getInt(R.styleable.PercentageRing_radiuso, 60);
		//字体颜色 默认为白色
...
//		mArcPaint.setStrokeWidth((float) (0.075*mRadius));
		mArcPaint.setStrokeWidth((float) (20));
	...
	public void setTargetPercent(int percent){
		...
		this.mCurrentAngle = (float) (360*(mTargetPercent * 1.0/100));
		Log.e("TAG", "mCurrentAngle="+mCurrentAngle+" mTargetPercent"+mTargetPercent);
		invalidate();
	}
